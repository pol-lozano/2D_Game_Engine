	...
	void updateLight() {
		...
		if (MOUSE_STATE == SDL_BUTTON(SDL_BUTTON_LEFT) || MOUSE_STATE == SDL_BUTTON(SDL_BUTTON_RIGHT)) {
			...
			case SDL_BUTTON(SDL_BUTTON_LEFT):
				blocks[std::clamp(_ty * CHUNK_SIZE + _tx, (Uint32)0, CHUNK_AREA - 1)].id = BlockID::GRASS;
				break;
...
public:	
		...
		for (Uint32 c = 0; c < Chunks.size(); c++) {
			...
			for (Sint16 ty = start_y; ty <= end_y; ty++) {
				for (Sint16 tx = start_x; tx <= end_x; tx++) {
					...
					if (_bl.light == 0) SDL_SetRenderDrawColor(rTarget, 55, 255, 255, 255);
	...
	//tilesheet, x for whatever tile id, y for whatever tile id, tileSIZE , position
	/*
	void drawSpriteClip(SDL_Texture tex, Uint16 srcR,) {
	drawSpriteCLip(tex, (BLOCK_LIST[_bl.id].tile.x + bl.sub) * TILE_SIZE, (BLOCK_LIST[_bl.id].tile.y) * TILE_SIZE,TILE_SIZE, tx,ty);
	}*/
private:
	SDL_Renderer* rTarget;
